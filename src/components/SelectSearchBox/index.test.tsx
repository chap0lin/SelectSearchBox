...
import SelectSearchBox from './index'
/**
...
test('should list data as options', async () => {
  render(<SelectSearchBox label="Teste" data={mockedData} onAddNew={(text)=>null} onInputChange={(text)=>null} />)
...
test('should call onAddNew when clicking in Adicionar novo', async () => {
  ...
  render(<SelectSearchBox label="Teste" data={[]} onAddNew={mockedAddNew} onInputChange={(text)=>null} />)
  await fireEvent.focus(screen.getByRole('searchbox'))
  await fireEvent.change(screen.getByRole('searchbox'), {target: {value: 'Ricardo Freitas'}})
